...
namespace FPSGame.src.Common{
    class TaskManager{
        private static readonly Lazy<TaskManager> _lazyVar = new Lazy<TaskManager>(() => new TaskManager());
        ...
        private TaskCompletionSource<bool> _tcsQueue = new TaskCompletionSource<bool>();
        private TaskManager(){
            this._maxParallelizationCount = System.Environment.ProcessorCount;
            Console.WriteLine("GLOBAL NUMBER OF CORES: {0}",this._maxParallelizationCount);
            this._maxQueueLength = int.MaxValue;
        }
        ...
        public bool QueueTask(Func<Task> task){
            if(_processingQueue.Count < _maxQueueLength){
                this._processingQueue.Enqueue(task);
                return true;
            }
        ...
        private void StartTasks()
        {
            ...
            if (_processingQueue.IsEmpty && _runningTasks.IsEmpty)
            {
                ...
                var _oldQueue = Interlocked.Exchange(
                    ref _tcsQueue, new TaskCompletionSource<bool>());
