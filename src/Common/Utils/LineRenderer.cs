...
namespace FPSGame.src.Common.Utils
{
     /// <summary>
 ...
 public class LineRenderer
    {
        ...
        public float lifetime {set; get;} = 5.0f;
        ...
        public void Update(Vector3[] points)
        {
            ...
            geometry.End();
            material.SetShaderParam("life",lifetime);
        }
        ...
        public void UpdateTime(float time){
            this.material.SetShaderParam("time",time);
        }
        public float getTime(){
            float val = (float)this.material.GetShaderParam("time");
            return val;
        }

        public void removeFromParent(){
            // this.geometry.GetParent().RemoveChild(geometry);
            this.geometry.QueueFree();
        }
        ...
        private void FillJoint(Vector3[] quad, int count)
        {
            ...
            {
                AddVertex(quad[0]);
                AddVertex((((quad[1].LinearInterpolate(quad[2], i / count)) - quad[3]).Normalized() * nextThickness) + quad[3]);
                AddVertex((((quad[1].LinearInterpolate(quad[2], (i + 1) / count)) - quad[3]).Normalized() * nextThickness) + quad[3]);
            }
        }
        ...
        {
            Color tmp = LerpColor(startColor, endColor, progress);
            geometry.SetColor(tmp);
