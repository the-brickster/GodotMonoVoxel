...
func _process(delta):
	...
	camPosText.text = str(self.transform.origin)
	...
	if(Input.is_action_pressed("sprint")):
		sprint_move_speed = 2.0
	elif(Input.is_action_just_released("sprint")):
		sprint_move_speed = 1.0
		print("released "+str(sprint_move_speed))
	...
	# Add motion
	velocity += motion*move_speed * sprint_move_speed
