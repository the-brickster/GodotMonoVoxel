...
void vertex() {
	...
	if (RESTART) {
		...
		vec3 direction_xz = vec3( sin(angle1_rad), 0, cos(angle1_rad));
		vec3 direction_yz = vec3( 0, sin(angle2_rad), cos(angle2_rad));
		direction_yz.z = direction_yz.z / sqrt(direction_yz.z); //better uniform distribution
	...
	COLOR = color_value * hue_rot_mat;

	if (length(VELOCITY) > 0.0) { TRANSFORM[1].xyz = normalize(VELOCITY); } else { TRANSFORM[1].xyz = normalize(TRANSFORM[1].xyz); }
